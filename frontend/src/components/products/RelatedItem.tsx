...
const ProductsItem: React.FC<ProductsItemProps> = ({ product }) => {
    ...
    // Reemplazar el último segmento con el slug del producto
    pathSegments[pathSegments.length - 1] = product.slug;

    // Unir los segmentos para formar la nueva URL
    const newUrl = pathSegments.join('/');

    return (
        <div className="border border-gray-200 rounded-md shadow-md hover:shadow-lg">
            <div className="w-full h-24 md:h-[20vh] overflow-hidden relative rounded-t-md">
                <Link href={newUrl}>
                    <Image
                ...
                <Buttons  product={product} quantities={quantities} handleDecreaseQuantity={handleDecreaseQuantity} handleIncreaseQuantity={handleIncreaseQuantity}/>                
            </div>
