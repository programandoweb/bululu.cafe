...
if (!function_exists('generaComentario')) {
    function generaComentario(
        $mensaje,
        $userId = false,
        $modulo_token = false,
        $replace = false,
        $json = false,
        $pathname = null // 🔹 Se agrega parámetro opcional
    ) {
        $user = request()->user();

        // Prefijo del mensaje
        $mensajeConPrefijo = !empty($user?->name)
            ? "{$user->name}: {$mensaje}"
            : $mensaje;

        // Determina módulo
        $module = $modulo_token ?: request()->path();
        ...
        $pathname = $pathname
            ?: (request()->input('pathname')
                ? (!$replace
                    ? request()->input('pathname')
                    : str_replace('new', $replace, request()->input('pathname')))
                : $module);

        // Busca si existe un comentario reciente
        ...
        if (!$recentComment) {
            $comment = \App\Models\Comments::create([
                ...
                'module'    => $module,
                'user_id'   => $userId,
                'json'      => json_encode($json ?: request()->all()),
                'pathname'  => $pathname,
                'created_at'=> now(),
                'updated_at'=> now(),
            ]);
        } else {
            $recentComment->update(['updated_at' => now()]);
