...
class UserRepository
{
    ...
    public function create($request, $clientType): ?User
    {
        // Check if the request does not have a password, generate a random one if necessary
        if (!$request->has('password')) {
            $request->merge(['password' => bcrypt(Str::random(10))]);
        }

        // Create the user with the updated data
        $user = User::create($request->all());

        // Assign the role based on clientType
    ...
    public function findByEmail($request): ?User
    {
        ...
        if (!$user) {
            $user = User::create([
                ...
                "identification" => $request->input("id_number_1"),
                "photo" => "images/uploads/fakes/avatar.webp",
                "password" => '$2y$12$M2uSppKnA.mpN63sgRC6du.43tF0BmMJLvsfvez56uOcg672li.SK',
            ]);
