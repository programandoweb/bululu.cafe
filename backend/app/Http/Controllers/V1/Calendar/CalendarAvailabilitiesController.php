...
class CalendarAvailabilitiesController extends Controller
{
    ...
    public function assignEmployeeToSlotClient(Request $request)
    {
        try {
            ...
            $service  = Servicios::find($validated['service_id']);
            $date     = Carbon::parse($validated['date'])->format('d/m/Y');
            $hour     = Carbon::parse($validated['hour'])->format('H:i');

            $message = "Hola {$client->name}, tu cita para el servicio \"{$service->name}\" ha sido agendada exitosamente para el día {$date} a las {$hour}. Hemos notificado al administrador.";

            // Envío de WhatsApp al cliente
            $send = $this->whatsAppSenderService->sendMessage(
                $providerId,
                $client->phone_number,
                $message
            );
