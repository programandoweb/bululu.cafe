...
class EventOrdersController extends Controller
{
        ...
        try {
            ...
            $formatted = [
                ...
                'Horario'   => [
                    ...
                    'cita_id' => $order->calendar_slot?->id,
                ],
            ];

            return response()->success(['order' => $formatted], 'Detalle de la orden status');
    ...
    {
        try {
            $order      = $this->eventOrderRepository->showByString($id);
            ...
            return response()->success(['order' => $formatted], 'Detalle de la orden show');
        } catch (\Exception $e) {
            return response()->error($e->getMessage(), 500);
        }
    }
    ...
    public function add_product(Request $request)
    {
        try {
            ...
            $data['provider_id']    =   $request->user()->customer_group_id ??$request->user()->id ?? null;
            $data['status']         =   "pendiente";

            $previewOrder   =       $this->eventOrderRepository->previewOrder($data);
            $random         =       md5($data['id']);
            ...
            $order = $this->eventOrderRepository->createOrUpdate($insert);

            $orders =   $this->eventOrderRepository->get($request,$order);
